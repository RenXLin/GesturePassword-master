...
- (void)drawRect:(CGRect)rect
{
    ...
    if (selected) {
        ...
        CGContextFillPath(context); //内部实心圆点：
    }
    else{
    ...
    CGRect frame = CGRectMake(2, 2, bounds.size.width - 4, bounds.size.height - 4);
    CGContextAddEllipseInRect(context,frame);
    CGContextStrokePath(context); //外围白色圆环
    ...
    if (selected) {
        CGContextFillPath(context);//选择的阴影：
    }
